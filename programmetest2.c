    ...
    exit(EXIT_FAILURE);
        ...
        if (compteur == 1){
            ...
            if ((compteur - 2) % 5 == 0){
                ...
                ajouter_coureur_fin(equipes, cycliste);
...
void ajouter_temps_etape(liste * liste, int nb_equipes){
    cellule * aux = liste->premier;
    int loop, random_number ;
    int temps ; int coureur_dope;
    struct timeval tv ;
    gettimeofday(&tv, NULL) ;
    srand(tv.tv_usec) ;
    for(loop = 1 ; loop <= (5*nb_equipes) ; loop++){
        coureur_dope = rand();
        if(coureur_dope <= (RAND_MAX / 20)){ // 5% de chance qu'un coureur soit dopé
            effacer_coureur(aux->coureur)

        }
        else{
            random_number = rand();
            while (random_number > 300){ //on considère que les étapes ne durent pas plus de 5h
                random_number = rand();
            }
            temps = random_number;
            ajouter_temps(aux->coureur, temps);
            aux = aux->suiv;
